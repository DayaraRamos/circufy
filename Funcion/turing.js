...
const bpts = Array.from({ length: 40 }, () => ({
...
(function bl() {
  ...
  bpts.forEach(p => {
    ...
    bgx.fillStyle = 'rgba(168,85,247,0.25)'; bgx.fill();
  });
  for (let i = 0; i < bpts.length; i++)
    for (let j = i + 1; j < bpts.length; j++) {
      const d = Math.hypot(bpts[i].x - bpts[j].x, bpts[i].y - bpts[j].y);
      if (d < 90) {
        bgx.strokeStyle = `rgba(168,85,247,${(1 - d / 90) * .04})`;
        bgx.lineWidth = .5; bgx.beginPath();
        bgx.moveTo(bpts[i].x, bpts[i].y); bgx.lineTo(bpts[j].x, bpts[j].y); bgx.stroke();
      }
    }
...
dc.addEventListener('mousedown', e => { const p = dcPos(e); dcDown(p.x, p.y); });
dc.addEventListener('mousemove', e => { const p = dcPos(e); dcMove(p.x, p.y); });
dc.addEventListener('mouseup',   () => dcUp());
dc.addEventListener('dblclick',  e => { const p = dcPos(e); dcDbl(p.x, p.y); });
...
// FIX: cancelar transFrom si se hace clic en canvas vacío en modo trans
dc.addEventListener('contextmenu', e => {
  e.preventDefault();
  if (transFrom) { transFrom = null; renderDiagram(); showToast('Selección cancelada', 'info'); }
});
  ...
  if (dtool === 'state') {
    ...
    if (hit) {
      if (!transFrom) {
        transFrom = hit;
        renderDiagram();
        showToast('Ahora haz clic en el estado destino (clic derecho = cancelar)', 'info');
      } else {
        document.getElementById('fFrom').value = transFrom.id;
        document.getElementById('fTo').value   = hit.id;
        transFrom = null;
        renderDiagram();
        showToast('Estados seleccionados — define la transición y pulsa AGREGAR', 'info');
      }
    } else {
      // FIX: clic en vacío cancela la selección pendiente
      if (transFrom) {
        transFrom = null;
        renderDiagram();
...
function dcMove(x, y) {
  if (dragState) { dragState.x = x - dox; dragState.y = y - doy; renderDiagram(); }
  if (dtool === 'move' && !dragState) dc.style.cursor = stateAt(x, y) ? 'grab' : 'default';
}
...
function dcDbl(x, y) {
  ...
  if (!hit.isInitial && !hit.isFinal) {
    ...
    hit.isInitial = true;
    showToast(hit.id + ' → INICIAL', 'ok');
  } else if (hit.isInitial && !hit.isFinal) {
    hit.isFinal = true;
    showToast(hit.id + ' → FINAL', 'ok');
  } else if (hit.isFinal && hit.isInitial) {
    hit.isInitial = false;
    showToast(hit.id + ' → solo FINAL', 'ok');
  } else {
    hit.isInitial = false;
    hit.isFinal   = false;
    showToast(hit.id + ' → normal', 'info');
  }
  updateStateSelects();
  renderDiagram();
}
...
function renderDiagram() {
  ...
  transitions.forEach((t, idx) => {
    ...
    const to   = states.find(s => s.id === t.to);
    if (!from || !to) return;
    drawTransitionArrow(dctx, from, to, t, idx === activeTransIdx);
  });
...
function drawTransitionArrow(ctx, from, to, t, isActive) {
  ...
  ctx.shadowColor = isActive ? '#ffe600' : 'transparent';
  ...
  if (isSelf) {
    ...
    ctx.fillText(label, from.x, from.y - 66);
    return;
  }
...
function drawState(ctx, s, isActive) {
  ...
  const gc  = isActive ? '#a855f7' : col;

  ctx.strokeStyle = gc; ctx.lineWidth = isActive ? 3 : 2;
  ctx.fillStyle   = isActive ? 'rgba(168,85,247,0.15)' : col + '12';
  ctx.shadowColor = gc; ctx.shadowBlur = isActive ? 24 : 8;
  ...
  ctx.shadowColor = isActive ? '#a855f7' : col;
...
function updateStateSelects() {
  ['fFrom','fTo'].forEach(id => {
    ...
    sel.innerHTML = states.map(s =>
      `<option value="${s.id}">${s.id}${s.isInitial?' →':''}${s.isFinal?' ✓':''}</option>`
    ).join('');
...
function addTransition() {
  ...
  if (exists) {
    showToast(`Ya existe transición desde ${from} leyendo "${read}" — elimínala primero`, 'bad');
    return;
  }
  ...
  renderDeltaTable();
  updateStateSelects();
  showToast(`✓ ${from} --[${read}/${write},${move}]--> ${to}`, 'ok');
}
...
function renderTransList() {
  ...
  list.innerHTML = transitions.map((t, i) => `
    <div class="trans-item${i === activeTransIdx ? ' active-trans' : ''}" id="titem-${i}">
      ...
      <span class="ti-rule">[${t.read}] wr:${t.write} mv:${t.move} → ${t.to}</span>
...
function renderTape() {
  ...
  if (!tape.length) {
    row.innerHTML  = '<span style="color:var(--dim2);font-size:0.65rem;padding:14px">Sin cinta. Ingresa una cadena y pulsa INICIAR.</span>';
    hrow.innerHTML = '';
    return;
  }
...
function initExecution() {
  // FIX: validar que haya máquina antes de cualquier otra cosa
  ...
  // FIX: si la cadena está vacía tratarla como epsilon (cinta con solo blancos)
  tape = str.length ? str.split('') : [];
  tape = ['B', 'B', ...tape, 'B', 'B'];
  head     = 2;
  curState = init.id;
  stepNum  = 0;
  ...
  clearConsole();
  log('SYS', `Cadena: "${str || 'ε'}" · Estado inicial: ${init.id}`, 'info');
...
// FIX: helper — verifica si la cinta (desde posición actual) tiene solo blancos
function tapeIsAllBlanks() {
  return tape.every(c => c === 'B');
}

function stepForward() {
  if (execDone) { showToast('La ejecución ya terminó. Presiona ↺ RESET', 'info'); return; }

  // FIX: verificar que la ejecución haya sido iniciada
  if (!tape.length) { showToast('Primero pulsa ▶ INICIAR EJECUCIÓN', 'bad'); return; }

  const tidx = getCurrentTransition();

  if (tidx < 0) {
    // Sin transición disponible
    const stateObj = states.find(s => s.id === curState);
    if (stateObj?.isFinal) {
      finalize(true);
    } else {
  ...
  // Guardar snapshot
  execHistory.push({
    tape: [...tape], head, curState, stepNum,
    activeTransIdx, tapeClone: JSON.stringify(tape)
  });

  const t       = transitions[tidx];
  const prevSym = tape[head] || 'B';
  const prevSt  = curState;

  tape[head] = t.write;
  head      += t.move === 'R' ? 1 : -1;

  // FIX: expandir cinta en ambas direcciones de forma segura
  ...
  log(stepNum, `${prevSt} · lee "${prevSym}" → escribe "${t.write}" · mv:${t.move} → ${t.to}`, 'transition');
  ...
  // FIX: solo aceptar automáticamente si estamos en final Y no hay más transición disponible
  const stateObj  = states.find(s => s.id === curState);
  if (stateObj?.isFinal) {
    const nextSym   = tape[head] || 'B';
    const nextTrans = transitions.find(tr => tr.from === curState && tr.read === nextSym);
    if (!nextTrans) {
...
function stepBack() {
  ...
  renderDeltaTable();
  renderTransList();
  document.getElementById('btnStepBack').disabled = !execHistory.length;
...
function runAll() {
  // FIX: si no se ha iniciado aún, iniciar automáticamente
  if (!tape.length) { showToast('Primero pulsa ▶ INICIAR EJECUCIÓN', 'bad'); return; }
  if (execDone)     { showToast('Reinicia primero con ↺ RESET', 'info'); return; }
  ...
  function tick() {
    ...
    if (tidx < 0) {
      const stateObj = states.find(s => s.id === curState);
      finalize(!!stateObj?.isFinal);
      return;
    }

    stepForward();

    // FIX: límite de pasos aumentado y con mensaje claro
    if (!execDone) {
      if (stepNum < 1000) {
        autoTimer = setTimeout(tick, delay);
      } else {
        log('SYS', '⚠ Límite de 1000 pasos alcanzado — posible bucle infinito', 'error');
        finalize(false);
      }
    }
  }
...
function setNavStatus(type, text) {
  ...
  // No sobreescribir el link de "Principal" — solo cambiar el dot
...
function loadPreset(name) {
  ...
  states         = [];
  transitions    = [];
  nextStateId    = 0;   // FIX: siempre usar nextStateId, nunca stateId
  activeTransIdx = -1;
  ...
  if (name === 'equal01') {
    ...
    showToast('Cargado: Suma unaria — "111 0 111" = "1111111"', 'ok');
  }
